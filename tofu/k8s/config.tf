...
resource "proxmox_virtual_environment_file" "k8s-ctrl-00" {
  provider     = proxmox.abel
  node_name    = var.abel.node_name
  content_type = "snippets"
  datastore_id = "local"
  ...
  source_raw {
    data = data.talos_machine_configuration.k8s-ctrl-00.machine_configuration
    #data = data.talos_machine_configuration.this.machine_configuration
    file_name = "talos-${var.k8s-ctrl-00.hostname}.cloud-config.yaml"
  }
}
...
#resource "proxmox_virtual_environment_file" "talos_cloud_config_worker" {
#  provider     = proxmox.abel
#  node_name    = var.euclid.node_name
#  content_type = "snippets"
#  datastore_id = "local"
#
#  source_raw {
#    data = data.talos_machine_configuration.machineconfig_worker.machine_configuration
#    file_name = "talos_worker.cloud-config.yaml"
#  }
#}
