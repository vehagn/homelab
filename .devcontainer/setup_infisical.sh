...
if [[ -v TF_VAR_infisical_domain ]] && [[ -n "$TF_VAR_infisical_domain" ]] &&
  ...
  infisical export --domain="$TF_VAR_infisical_domain" --projectId="$TF_VAR_infisical_project_id" --path="$TF_VAR_infisical_rw_secrets_path" --format=dotenv-export >> "$HOME/.infisical_exports.env"
  if [ $? -ne 0 ]; then
